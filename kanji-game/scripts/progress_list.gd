...
func add_char(char: String, r_progress: int, w_progress: int) -> void:
	...
	add_progress_label(r_progress,0)
	add_progress_label(w_progress,1)

func add_progress_label(value: int, state: CHAR_STATE):
	var label: Label = Label.new()
	label.text = str(value) + "/6"
	label.vertical_alignment = VERTICAL_ALIGNMENT_CENTER
	label.horizontal_alignment = HORIZONTAL_ALIGNMENT_CENTER
	match state:
		0:
			read_progress.add_child(label)
		1:
			write_progress.add_child(label)
